...
if(isset($_POST["submit"])){
  ...
  $duplicate = mysqli_query($conn, "SELECT * FROM user_input WHERE username = '$username' OR email = '$email'");
  if(mysqli_num_rows($duplicate) > 0){
    echo
    "<script> alert('Username or Email Has Been Used'); </script>";
  }
  else{
    if($password == $confirmpassword){
      $hashed_password = password_hash($password, PASSWORD_DEFAULT); //hash the password
      $query = "INSERT INTO user_input VALUES('','$name','$username','email','$hashed_password')";
